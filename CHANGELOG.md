...
## 1.0.17 — 2026-09-02（html 表格化重构 + 呈现收尾；纯呈现层）

诊断逻辑 / md 表格结构 / 探针 JSON 契约一律未动。

**表格直接展示（接卡片/折叠版）**

- 按用户反馈彻底去掉卡片与折叠，全部改为白底表格直接展示：绿色结论 hero → 核心问题面板 → 先做表 → 先停表 → 每页一词表 → 技术核查表。
- 核心问题面板从 `findings[rule==priority[0]].focus.share` 取真实占比做大字号 stat（`priority` 项本身无 `focus` 字段，须回 findings 查）。peercare 渲染为「75% 的页面是博客文章」。
- 修漏：先用 `focus_reading`（含英文 posts）→ 门禁报黑话泄漏；改回用已中文化的 `_facts_rows` 文本，不再泄漏英文。
- 门禁误报根因：预览工具给盘上 html 注入 `data-page-node-id` 属性，使 `check_sample_consistency` 报「html 与 json 不同源」；剥掉属性后盘上文件与 json 重渲逐字一致，渲染本身无漂移（纯环境包裹，非代码缺陷）。
- 四道门禁 + 形态（本仓 4 个 .py 超标 0）全绿（在干净文件上）。
- **修复技术核查表内容缺失**：原 `_html_tech` 只渲染 `_site_notes`（8 条软性说明），json 里真正逐项核查的 18 条 `findings`（含 status + evidence）一项都没列。改为逐条列出 findings（状态 | 检查项 | 证据），状态中文映射（通过/未通过/注意/检测到/没看到，na≠pass），证据列过 `_no_leak` 把黑话 posts→博客文章，避免门禁 blacklist 泄漏；`_site_notes` 降为「补充说明」小节。
- 顺手修：删除重复的旧 `_html_table(rows,headers)` 死定义；`_html_style` 从 44 行压到 ~9 行（修复 shape 超标）。

**呈现收尾（承接表格版）**

- **页面加宽**：`body max-width` 820px → 1000px，表格横向更舒展。
- **去掉「核心问题」面板左侧 4px 粗色边**（用户：不要边缘加粗效果），保留白底细边框圆角。
- **html 补齐 md「八、需要你手动处理」对应节**：新增 `_html_manual`（待办任务表：待办任务 | 为什么探针查不了；需你确认表：需你确认 | 为什么探针看不到，数据源 `_cannot_todo` / `_cannot_confirm`，行内按「 —— 」拆任务与原因）与 `_html_next`（「还需补充的数据」单列表，数据源 `_next_rows`），插在技术核查之后、footer 之前，与先做/先停并列。空数据时整节不渲染。
- **修复 `<b>不要编</b>` 二次转义**：根因是表格版 `_html_table` 对所有单元格无条件 `_esc()`，而 `_html_stop` 把已含真实 `<b>` 的 `_bold_no_fake(r[3])` 传进去 → 再被转义成 `&lt;b&gt;`。改：`_html_table` 增加 `raw_cols` 参数（按列索引跳过转义），先停表第四列（边界在哪）标为 raw。
- **修复潜伏双重转义**：`_html_do` / `_html_keyword` / `_html_manual` / `_html_next` / `_html_tech` 在单元格层面先 `_esc`、再进 `_html_table` 又 `_esc`，形成二次转义（当前内容无 `& <` 时不显形）。改：`_html_table` 为唯一转义点，调用侧传原文；`_no_leak` 改为仅做 `posts→博客文章` 替换，不再预转义，统一由表格转义。
- 门禁同步：`tests/report_gate.py` 的 `NEED` 增 `("需你手动处理","html",...)`，保证该节在 html 中真实出现（改生成器必改断言，不删断言）。
- 验证：report_gate / confidence_gate / fetch_retry_gate 全过；shape_check 本仓 4 个 .py 超标 0；html 中 `&lt;b&gt;` 计数 0、真实 `<b>` 2（demo 提示 + 不要编）；剥掉预览注入的 `data-page-node-id` 后盘上 html 与 json 重渲逐字一致。

## 未发布（html 卡片化 90 分版：老板卡片 + 技术折叠 + 词表示范）

纯渲染层改造，诊断逻辑 / md 表格结构 / 探针 JSON 契约一律未动。目标：把报告从「75 分（数据对得上）」推到「90 分（完全不懂 SEO 的一般人也能 3 秒抓重点、不会去修假问题）」。

- **先做 / 先停 拆成两种读者视图**：老板/运营看 3 张卡片（你是谁？/ 哪几页赚钱？/ 每页排什么？），验收语口语化；技术细节整体收进默认收起的 `<details>`（标题「展开看完整待办清单（N 条）」），不再压在运营前面把人劝退。
- **每页要排哪个词 填 1 个示范**：首页行直接给「示范：内部叫『伯吹童韵』→ 外人搜『幼儿园本土文化课程设计 / 二十四节气校本课程』」，点破内部叫法 vs 外人搜的词；另加黄色 demo 框（❌ 内部叫法 / ✅ 外人会搜）。空表没人填的问题闭环。
- **口语化**：「能产生订单/咨询的页面」→「能赚钱的页面」；「每页只认一个要排的词」→「一页只抢一个词，别贪心」；「不要编」加粗为 `<b>不要编</b>`（先转义再插标签，修掉上一轮 `&lt;b&gt;` 显示 bug）。

门禁同步（改生成器必改断言，不删断言）：
- `tests/report_gate.py` 的 `NEED` 元组改写：删 `("<table","html"...)`、`("谁改","html"...)`；新增 `("你是谁","html"...)`、`("<details","html"...)`、`("先停","html"...)`、`("能赚钱的页面","md"...)`、`("示范","html"...)`、`("一页只抢一个词","html"...)`、`("每页要排哪个词","html"...)`。md 的 `| 谁改 |` / `| # | 别碰哪块 |` 表头保留（md 结构没动，md 门禁不破）。

验证：report_gate / confidence_gate / fetch_retry_gate 全过；shape_check 本仓库 4 个 .py 超标 0（顶层 `/Users/tong/SEO/` 的陈年超标不属本仓，已定性不挡结案）；html 12 个关键节点（你是谁 / 哪几页赚钱 / 每页排什么 / 技术自检已通过 / 示范：内部叫 / 一页只抢一个词 / 能赚钱的页面 / 先停（这些先别做）/ 不要编 / ❌ / ✅ / <details）全命中。样例 json 加 `home_keyword_demo`，md/html 按新生成器重渲，`check_sample_consistency` 绿。

未动两条历史残留（用户判不挡结案）：`sampled-originality` 分类错（audit_url.py:978 ACTIONS 内）、75% 博客重复（md:3 与 md:30）。
...
- `render_html` 读法句此前写死「技术没硬伤时，先看先停」，md 已按 verdict 分流、html 没有。critical 的 html 会自相矛盾。改为调用 `_reading_line`；第十节与 md 对齐。`check_reading` 同时断言 md 和 html。
