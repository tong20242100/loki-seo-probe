...
- **m-host 的 NXDOMAIN 判定改用真实 DNS**：原按 urllib 错误文本正则分流，而错误文本随代理/本地化变——peercare.cn 同一主机、同一站点事实（socket 直查 errno=8 确认无 m 站），一次报 `nodename`→pass 真阳性，一次因沙箱代理隧道报 `Tunnel connection failed: 502`→文本不匹配→误判 na。**同一个站两种结论**，是 P9「抖动不该翻结论」的翻版。新增 `host_resolves()` 走 `socket.getaddrinfo`（不受 HTTP 代理影响），`gaierror`=确认没有 m 站→pass，解析得到但连不上=na。5xx/200 仍 warn。
