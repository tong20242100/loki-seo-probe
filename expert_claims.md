# Loki Yan SEO 主张表（判定规则的来源）

> **134 条**主张，全部出自 @loki_yan_seo 本人推文（2026-02-11 ~ 2026-08-29）。
> 收录标准：本人所写（转推、引用他人观点不进）、主题与 SEO 相关且为核心议题、
> 信号置信 strong（转引一律不进）。未收录：转推、引用混合、待复核、短回复。
> 本表是 SKILL.md 的前置筛料：主张站得住才进 skill，站不住的不进。
> 出处列 `#n` 对应文末索引表；原文全文见 `corpus.json`（#n 同序）。
> **边界**：本表是按上述标准对语料快照（截至 2026-08-29）的筛选结果，不是全账号；
> 此后的推文不在内。
> **版权**：原文归原作者所有，收录仅为可回源核验；非官方实现、无授权无背书。
...
| 标记 | 含义 |
|---|---|
| **进** | 反复出现（≥3 条或 ≥2 个独立场合）+ 有一手证据 → 可写成判定规则 |
...
- 判定：**降** —— 亲历代价（20K USD/月、YMYL）可进；「停了无损失」不能当通用规则。
  n=1 且他自己点了混杂因素。判定规则只能写成「别把流量涨跌单归因到外链」
...
- 判定：**进**（把 KD 从「词的难度」改成「对面编制」）；编制数字不写进判定规则当事实
...
- 判定：**进**。判定规则里点名为 ranking factor 的是 **Quality**，不要写成「EEAT 是 ranking factor」。
...
- 判定：**进**（机制段进）；「没做 Reddit/schema/FAQ 也能涨」是 **n=1 他自己的站**，
  判定规则里只作反例（反商品化 GEO 清单），不当成「这些都不必做」
...
- 判定：**降** —— 开放问题，他自己在问。判定规则只作刹车：不要把经典 CTR / Navboost 直接套到 AIO。
  不当定律。
...
- 判定：**进（搜集工具，不另开判定规则）** —— skill 的「核不到怎么搜集」段用。
  不把「去装 Frog」写成第 8 条技术判定规则。
...
- 判定：**进**（#3 + #42 的页级动作）；「小半径更容易成功」仍是推论，判定规则里必须标推论
...
## 附录B：风险信号清单（自正文 134 条判定规则反推）

> 每条右侧是正文判定规则出处，可回查对应判定规则与 `#n`；与正文冲突时以正文为准。
> 标「语料无」的条目＝语料只点名、未给修法，不得编 how-to。
...
| IP 自动切语言 / 自动跳转 | 7.2（cloaking 相邻）；语料无 how-to |
...
| 千人千面 / 个性化 vs 爬虫看到哪一面 | 7.2（cloaking 相邻）；语料无 how-to |
| 内链带追踪参数（有 canonical 也算） | 语料无 |
...
| GSC AI citations / Bing Total Citations 当成功指标 | 2.x |
| GBP / 本地列表无站内 NAP | 语料无 |
