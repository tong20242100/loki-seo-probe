...
主张全文 `expert_claims.md`（本包同目录，`#n` → 文末 tweet_id）。本文件是**调用规程**：探针 → 选口诀 → 落地动作。禁止把下面口诀全倒进回复。
...
| 「怎么做 SEO」太宽 | 探针 fail/warn → 最多 3–7 条 | 口诀全文 |
...
只采 `core × authored × strong × is_main`。常识（robots 第一、H1 放目标词、301）不进口诀；下跌时用**探针读 robots**，不新开口诀。

## 红旗：命中即停 / 先拆（选口诀之前先过一遍）

来源 `expert_claims.md` 附录B（第三方蒸馏的 RF01–RF28），已逐条对照「明确不进」。
**能对的接进上表口诀；冲突的写「不采用」并给理由；语料没有的标 `no-procedure`——点名可以，不许编 how-to。**
...
- **RF18「H1 不通顺 / H1 不是排名词」— 整条不采用。** 冲突点：「H1 放目标词」是我们明确不进口诀的常识。我们只在 6.3 讲「这页说不出自己排什么」，不讲 H1 必须放哪个词。（2026-09-01 复核钉死：#125＝2093224389605261381 在 134 内、G7 已判「常识·不进口诀」，跟帖 2093236681449332851 亦在库——**不采用是审过，不是漏采**，勿再当新缺口。诗意空 H1 的落点＝一页一词表里「这页说不出自己排什么」，作 6.3 反例，不升「H1 放目标词」口诀。）
...
5. **证据等级**：口诀进 / 口诀降 / 开放问题 / 探针事实 / 无数据。标题带`（降）`的，边界写进该条动作。
...
禁止：走完全部口诀；编造 tweet_id；把降级案例升级成定律；GEO 作业清单；把 DA/citations 当原因。

发稿前自查（合同闸门，改自 lynnzc 敌意自审、非新能力；命中即改，改不了的在证据等级行暴露）：**编造**——id / 数字 / 站点名无源；**进口**——常识 checklist 混进口诀（密度榜、外链建设、GEO 作业、title 字符数）；**升格**——（降）/ 单点 / asserted 被写成定律；**传记**——用户要审站、回复在讲 Loki 本人；**GEO 幻觉**——语料明说无 how-to 的领域给出了作业清单。
